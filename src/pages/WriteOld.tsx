...
export const WriteOld: React.FC = () => {
    ...
    try {
        ...
        try {
          ...
          if (typeof maybeAuthStateReady === 'function') {
            try {
              ...
              console.warn('⚠️ [WriteOld] authStateReady check failed', readyError);
            }
          }
          ...
          return await new Promise<string | null>((resolve) => {
            ...
            unsubscribe = onAuthStateChanged(
              ...
              (listenerError) => {
                clearTimeout(timeoutId);
                console.warn('⚠️ [WriteOld] Auth listener error while resolving token', listenerError);
                finalize(null);
              }
            );
          });
        } catch (tokenError) {
          console.warn('⚠️ [WriteOld] Unexpected error resolving auth token', tokenError);
          return null;
        }
      };
      ...
      try {
        ...
        if (token) {
          ...
          console.log('🔐 [WriteOld] Attached auth token for script persistence');
        } else {
          console.warn('⚠️ [WriteOld] No auth token available; script will be stored locally');
        }
      } catch (tokenError) {
        console.warn('⚠️ [WriteOld] Failed to retrieve auth token for script persistence', tokenError);
      }
      ...
      if (!response.ok || !data?.success) {
        console.warn('⚠️ [WriteOld] Failed to persist generated script', {
      ...
      console.log('💾 [WriteOld] Generated script saved', data.script?.id);
      return data.script ?? null;
    } catch (err) {
      console.error('❌ [WriteOld] Persisting script failed', err);
      return null;
    }
  }, [brandVoices]);
...
"Try this method for just one week and tell me your results! Follow for more ${request.platform} tips ✨"`,
  ...
  const handleGenerate = async (request: AIGenerationRequest) => {
    console.log("🎬 [WriteOld] handleGenerate called with request:", request);
    ...
    try {
      ...
      console.log("📏 [WriteOld] Length mapping:", { original: request.length, mapped: mappedLength });
      
      console.log("🔄 [WriteOld] Calling generateScript...");
      ...
      console.log("📋 [WriteOld] Generate script result:", result);

      if (result.success && result.script) {
        console.log("✅ [WriteOld] Script generation successful, creating content...");
...
${result.script.wta}`;
        ...
        console.log("📝 [WriteOld] Generated script content:", scriptContent);
        ...
        console.log("🧭 [WriteOld] Navigating to:", editorUrl);
        
        navigate(editorUrl);
        console.log("✅ [WriteOld] Navigation completed");
      } else {
        console.error("❌ [WriteOld] Script generation failed or incomplete:", result);
      }
    } catch (error) {
      console.error('❌ [WriteOld] Generation failed with exception:', error);
