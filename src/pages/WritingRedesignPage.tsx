    ...
    try {
      ...
      if (typeof maybeAuthStateReady === 'function') {
        try {
          ...
          console.warn('⚠️ [WritingRedesignPage] authStateReady check failed', readyError);
        }
      }
      ...
      return await new Promise<string | null>((resolve) => {
        ...
        unsubscribe = onAuthStateChanged(
          ...
          (listenerError) => {
            clearTimeout(timeoutId);
            console.warn('⚠️ [WritingRedesignPage] Auth listener error while resolving token', listenerError);
            finalize(null);
          }
        );
      });
    } catch (tokenError) {
      console.warn('⚠️ [WritingRedesignPage] Unexpected error resolving auth token', tokenError);
      return null;
    }
  }, []);
  ...
  const persistGeneratedScript = useCallback(async (
    ...
    try {
      ...
      if (token) {
        ...
        console.log('🔐 [WritingRedesignPage] Attached auth token for script persistence');
      } else {
        console.warn('⚠️ [WritingRedesignPage] No auth token available; script will be stored locally');
      }
    } catch (tokenError) {
      console.warn('⚠️ [WritingRedesignPage] Failed to retrieve auth token for script persistence', tokenError);
    }
    ...
    try {
      ...
      if (!response.ok || !data?.success) {
        console.warn('⚠️ [WritingRedesignPage] Failed to persist generated script', {
      ...
      console.log('💾 [WritingRedesignPage] Generated script saved', data.script?.id);
      return data.script ?? null;
    } catch (error) {
      console.error('❌ [WritingRedesignPage] Persisting script failed', error);
      return null;
    }
  }, [resolveAuthToken]);
