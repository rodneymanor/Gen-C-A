...
const CACHE_MAP_KEY = '__conditionalHooksFactory_conditionCacheMap_dont_modify_this_manually_unless_you_want_react_to_blow_up';
...
export function conditionalHooksFactory(condition, newHook, oldHook) {
  ...
  // Clean up with platform_editor_global_conditional_factory_cache
  let globalCacheGateCache = null;
  const globalCacheHookFn = (...args) => {
    // call each time so we can track exposures
    const conditionResult = condition();
    const cache = globalThis[CACHE_MAP_KEY];
  ...
  }, globalCacheHookFn, localCacheHookFn);
}